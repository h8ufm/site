					...
					if (isset(conf[name]) && isset(conf[name].cfg.antispamjs)) {
						$(form).prepend(
							'<input type="text" name="' +
								conf[name].cfg.antispamjs +
								'" value="tesby" style="display:none;">'
						);
					}
				});
			}
					...
					if (/textbox/i.test(cfg.notify)) {
						...
						if (isset(answer.infos)) {
							$.each(answer.infos, function (k, val) {
								$(".agree label").removeClass("check");
								$("#popup_thanks").addClass("popup_active");
								$("#popup_thanks .popup_thanks").addClass(
									"popup_active"
								);
								$(".popup").removeClass("_active");
								setTimeout(function () {
									$(
										"#popup_thanks .popup_thanks"
									).removeClass("popup_active");
								}, 2500);
								setTimeout(function () {
									$("#popup_thanks").removeClass(
										"popup_active"
									);
								}, 2500);
								$(".input-name").val("");
